...
def make_image(text: str, subtitle: str) -> np.ndarray:
    ...
    for y in range(img.shape[0]):
        val = int(25 + (y / img.shape[0]) * 60)
        img[y, :, :] = (val, val, val)
    cv2.rectangle(img, (30, 30), (610, 450), (85, 85, 85), 2)
    cv2.putText(img, text, (60, 180), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (245, 245, 245), 3, cv2.LINE_AA)
    cv2.putText(img, subtitle, (60, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (210, 210, 210), 2, cv2.LINE_AA)
    cv2.putText(img, "SIMULATED SNAPSHOT", (60, 420), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (185, 185, 185), 2, cv2.LINE_AA)
    return img

def main():
    parser = argparse.ArgumentParser(description="Generate simulated snapshot images + DB rows (NO hardware).")
    parser.add_argument("--n", type=int, default=12, help="Number of snapshots to generate")
    ...
    for i in range(args.n):
        ...
        snapshot_path = f"snapshots/{relpath}"

        if args.link_alerts and random.random() < 0.7:
            ...
            linked_alert_id = create_alert(
                ...
                status="ACTIVE",
                details=f"auto-linked sim alert for {stype}",
